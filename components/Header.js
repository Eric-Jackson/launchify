...
export default function Header({ theme = "light" }) {
  ...
  return (
    <nav className={`w-full px-6 py-4 ${themeStyles.header} backdrop-blur-md sticky top-0 z-50`}>
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        <Link href="/" className="flex items-center gap-2 text-xl font-bold">
          <img 
            src="/launchify-logo.svg" 
            alt="Launchify logo" 
            className={`w-auto max-h-[50px] ${theme === 'dark' ? 'invert brightness-0' : ''}`} 
          />
        </Link>

        <div className="sm:hidden">
          <button onClick={() => setOpen(!open)} aria-label="Toggle Menu">
            {open ? <X className={`w-6 h-6 ${themeStyles.textPrimary}`} /> : <Menu className={`w-6 h-6 ${themeStyles.textPrimary}`} />}
          </button>
        </div>

        <div className={`hidden sm:flex items-center space-x-6 text-sm ${themeStyles.textSecondary}`}>
      ...
      {showMenu && (
        <div className={`absolute top-full left-0 w-full ${themeStyles.header} border-t shadow-md z-40 sm:hidden ${open ? 'animate-slide-down' : 'animate-slide-up'}`}>
          <div className={`flex flex-col px-6 py-4 space-y-2 text-sm ${themeStyles.textSecondary}`}>
